...
if type == "spc":
	...
	cur.execute("SELECT id, image_url, pid  FROM orchid_spcimages where image_url <>'' and image_url is not null and (image_file is null or image_file = '') and pid >= 0 order by pid")
else:
	thumbdir = dir + "hybrid_thumb/"
	imgdir  = dir + "hybrid/"
	stmthead = "UPDATE orchid_hybimages set image_file = '%s', width=%d, height=%d where id = %d"
	cur.execute("SELECT id, image_url, pid  FROM orchid_hybimages where image_url <>'' and image_url is not null and (image_file is null or image_file = '') order by pid")
